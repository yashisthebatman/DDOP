...
class PathPlanner3D:
    # ... (init, build_abstract_graph, _get_cost_function, find_path are unchanged and correct)
    ...
    def perform_hybrid_replan(
        self, current_pos: Tuple, goal_pos: Tuple, new_obstacle_bounds: Tuple, 
        payload_kg: float, mode: str, time_weight: float = 0.5
    ) -> Tuple[Optional[List[Tuple]], str]:
        """
        Performs a robust, two-stage replan.

        1.  **Tactical Escape:** Finds a path from the drone's current position to the
            nearest safe and reachable node on the abstract graph.
        2.  **Strategic Re-route:** Plans a new high-level path from that safe
            node to the final destination.
        """
        logging.info("--- Starting Hybrid Replan ---")
        
        # Invalidate the strategic graph first
        self.invalidate_abstract_graph_edges(new_obstacle_bounds)

        # --- Step 1: Find a Tactical Escape Route ---
        ...
        if not escape_goal_node:
            return None, "Fatal: Drone is trapped. No safe node on the abstract graph is reachable."

        logging.info(f"Found safe escape goal: {escape_goal_node}")
        
        # Use D* Lite to find the path to this safe intermediate goal
        escape_path, status = self._find_tactical_path(current_pos, escape_goal_node, new_obstacle_bounds)

        if not escape_path:
            return None, f"Tactical escape failed: {status}"
        
        logging.info(f"Tactical escape path with {len(escape_path)} waypoints found.")

        # --- Step 2: Plan from the Escape Point to the Final Destination ---
        ...
        if not final_dest_node:
            return None, "Strategic Replan Failed: Could not find network node near final destination."

        try:
            cost_func = self._get_cost_function(payload_kg, mode, time_weight)
            heuristic_func = self.heuristics.get_heuristic(mode, final_dest_node, payload_kg, time_weight)
            
            # Plan from the end of the escape route to the final destination
            strategic_path_nodes = nx.astar_path(
                self.abstract_graph,
                source=escape_goal_node,
                target=final_dest_node,
                weight=cost_func,
                heuristic=heuristic_func
            )
            
            # Combine the paths: Escape Path -> Strategic Path -> Final Destination
            ...
            return None, "Strategic Replan Failed: No path exists from escape node to destination."
        except Exception as e:
            logging.exception("An unexpected error occurred during strategic replan stage.")
            return None, f"Strategic replan error: {e}"

    def _find_tactical_path(self, start_pos, end_pos, new_obstacle_bounds):
        """Helper to run D* Lite for a short-range path."""
        ...
        dstar = DStarLite(
            ...
            heuristic_provider=self.heuristics, coord_manager=self.coord_manager, mode='time'
        )
    ...
    def _find_nearest_safe_abstract_node(self, pos, obstacle_bounds):
        """Finds the closest node on the abstract graph that is reachable and not inside the new obstacle."""
        ...
        for idx in np.argsort(distances):
            ...
            # Condition 1: Node must not be inside the new obstacle
            if not point_in_aabb(node, obstacle_bounds):
                # Condition 2: Must have a clear line of sight from the drone
                if not self.env.is_line_obstructed(pos, node):
                    return node
        return None

    # ... (Other helper methods are unchanged and correct)
