# FILE: simulation/contingency_planner.py
import logging
import uuid
import numpy as np
from typing import Dict, Any

from config import HUBS, RTH_BATTERY_THRESHOLD_FACTOR, DRONE_BASE_POWER_WATTS, DRONE_ADDITIONAL_WATTS_PER_KG
from utils.geometry import calculate_distance_3d
from planners.single_agent_planner import SingleAgentPlanner

def log_event(state, message):
    import time
    state['log'].insert(0, f"{time.strftime('%H:%M:%S')} - {message}")

def _find_nearest_hub(pos, coord_manager):
    pos_m = coord_manager.world_to_meters(pos)
    hubs_with_dist = []
    for hub in HUBS:
        hub_pos_m = coord_manager.world_to_meters(hub['location'])
        dist = calculate_distance_3d(pos_m, hub_pos_m)
        hubs_with_dist.append((dist, hub['id'], hub['location']))
    if not hubs_with_dist: return None, None
    _, nearest_hub_id, nearest_hub_pos = min(hubs_with_dist, key=lambda x: x[0])
    return nearest_hub_id, nearest_hub_pos

def _trigger_emergency_return(state: Dict, drone_id: str, reason: str, planners: Dict):
    drone = state['drones'][drone_id]
    sim_time = state['simulation_time']
    if drone['status'] in ['EMERGENCY_RETURN', 'CRITICAL_FAILURE']: return
        
    original_mission_id = drone['mission_id']
    log_event(state, f"⚠️ CONTINGENCY: {drone_id} entering EMERGENCY_RETURN due to: {reason}.")
    
    if original_mission := state['active_missions'].get(original_mission_id):
        current_stop_index = original_mission.get('current_stop_index', 0)
        stops_to_requeue = original_mission.get('stops', [])[current_stop_index:]
        
        orders_returned = 0
        for order_details in stops_to_requeue:
            if (order_id := order_details['id']) not in state['pending_orders']:
                state['pending_orders'][order_id] = order_details
                orders_returned += 1
        
        if orders_returned > 0:
            log_event(state, f"Returned {orders_returned} orders from failed mission {original_mission_id} to queue.")
            
        log_entry = { "mission_id": original_mission_id, "drone_id": drone_id, "completion_timestamp": sim_time, "outcome": f"Failed: {reason}", "planned_duration_sec": original_mission.get('total_planned_time', 0), "actual_duration_sec": sim_time - original_mission.get('start_time', 0), "planned_energy_wh": original_mission.get('total_planned_energy', 0), "actual_energy_wh": original_mission.get('start_battery', 0) - drone['battery'], "number_of_stops": len(original_mission.get('stops', [])) }
        state['completed_missions_log'].append(log_entry)
        del state['active_missions'][original_mission_id]

    drone['status'] = 'EMERGENCY_RETURN'
    
    coord_manager = planners['coord_manager']
    hub_id, hub_pos = _find_nearest_hub(drone['pos'], coord_manager)
    if not hub_pos:
        drone['status'] = 'CRITICAL_FAILURE' 
        return
        
    planner = SingleAgentPlanner(planners['env'], planners['predictor'], coord_manager)
    path, status = planner.find_strategic_path_rrt(drone['pos'], hub_pos)
    if not path:
        drone['status'] = 'CRITICAL_FAILURE'
        return
        
    total_time, total_energy = 0, 0
    if len(path) > 1:
        for i in range(len(path) - 1):
            p1, p2 = path[i], path[i+1]
            wind = planners['env'].weather.get_wind_at_location(*p1)
            t, e = planners['predictor'].predict(p1, p2, 0, wind)
            total_time += t; total_energy += e
            
    emergency_mission_id = f"EM-{uuid.uuid4().hex[:6]}"
    emergency_mission = { 'mission_id': emergency_mission_id, 'drone_id': drone_id, 'order_ids': [], 'stops': [], 'start_pos': drone['pos'], 'destinations': [hub_pos], 'payload_kg': 0, 'path_world_coords': path, 'total_planned_time': total_time, 'total_planned_energy': total_energy, 'start_time': sim_time, 'start_battery': drone['battery'], 'mission_time_elapsed': 0.0, 'flight_time_elapsed': 0.0, 'total_maneuver_time': 0, 'end_hub': hub_id }
    state['active_missions'][emergency_mission_id] = emergency_mission
    drone['mission_id'] = emergency_mission_id

def check_for_contingencies(state: Dict, planners: Dict, drone: Dict) -> bool:
    if drone['status'] not in ['EN ROUTE', 'RETURNING_TO_HUB', 'PERFORMING_DELIVERY']:
        return False
    mission = state['active_missions'].get(drone['mission_id'])
    if not mission: return False

    env = planners['env']
    predictor = planners['predictor']
    
    energy_to_finish_mission = 0
    if drone['status'] == 'PERFORMING_DELIVERY':
        remaining_maneuver_time_s = max(0, mission.get('maneuver_complete_at', 0) - state['simulation_time'])
        hover_power_watts = DRONE_BASE_POWER_WATTS + (mission.get('payload_kg', 0) * DRONE_ADDITIONAL_WATTS_PER_KG)
        energy_to_finish_mission = (hover_power_watts * remaining_maneuver_time_s) / 3600.0
        pos_for_return_calc = mission['stops'][mission['current_stop_index']]['pos']
    else:
        pos_for_return_calc = mission['destinations'][-1]
        payload = mission.get('payload_kg', 0)
        _, energy_to_finish_mission = predictor.predict(drone['pos'], pos_for_return_calc, payload, env.weather.get_wind_at_location(*drone['pos']))
    
    _, nearest_hub_pos = _find_nearest_hub(pos_for_return_calc, planners['coord_manager'])
    if nearest_hub_pos:
        _, energy_to_return_to_hub = predictor.predict(pos_for_return_calc, nearest_hub_pos, 0, env.weather.get_wind_at_location(*pos_for_return_calc))
        required_energy = (energy_to_finish_mission + energy_to_return_to_hub) * RTH_BATTERY_THRESHOLD_FACTOR
        if drone['battery'] < required_energy:
            _trigger_emergency_return(state, drone['id'], "Critically Low Battery", planners)
            return True

    # FIX: The check now consumes the flag.
    if env.was_nfz_just_added:
        env.was_nfz_just_added = False # Consume the flag
        if path := mission.get('path_world_coords', []):
            current_idx = np.argmin([np.linalg.norm(np.array(drone['pos']) - np.array(p)) for p in path])
            for i in range(max(0, current_idx - 1), len(path) - 1):
                if env.is_line_obstructed(path[i], path[i+1]):
                    _trigger_emergency_return(state, drone['id'], "Path Invalidated by NFZ", planners)
                    return True
                    
    return False