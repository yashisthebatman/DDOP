...
from config import MIN_ALTITUDE, NO_FLY_ZONES
...
def test_point_obstruction(fresh_environment):
    ...
    env = fresh_environment
    # This point is inside the first static NFZ: [-74.01, 40.715, -73.995, 40.725]
...
def test_line_obstruction(fresh_environment):
    ...
    # This line crosses the first static NFZ
...
def test_dynamic_nfz_management(fresh_environment):
    ...
    # Dynamic zone is: [-74.005, 40.72, -73.995, 40.73]
    # FIX: Use a point that is ONLY inside the dynamic zone, not also a static one.
    point_only_in_dynamic_nfz = (-74.00, 40.728, 50.0)
    assert env.is_point_obstructed(point_only_in_dynamic_nfz)
    
    # Test removal
    env.remove_dynamic_obstacles()
    assert len(env.dynamic_nfzs) == 0
    assert len(env.obstacles) == initial_obstacle_count
    # Assert that the point is now clear
    assert not env.is_point_obstructed(point_only_in_dynamic_nfz)