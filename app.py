...
st.title("🚁 Q-DOP: Pure QUBO Drone Path Optimization")
for key, default in [('stage', 'setup'), ('mission_plan', None), ('animation_step', 0), ('log', [])]:
    if key not in st.session_state: st.session_state[key] = default

# --- STAGE 1: Mission Setup UI (Unchanged) ---
def setup_stage():
    ...
    if st.button("🚀 Plan Mission with Pure QUBO", type="primary", use_container_width=True):
...
# --- STAGE 2: Path Planning Execution ---
def planning_stage():
    log_event("Starting mission planning with Pure QUBO...")
    with st.spinner("Formulating and solving the QUBO model... This is computationally intensive and may take some time."):
        ...
        if path_to is None or path_from is None:
            st.error(f"Fatal Error: Path planning failed. Status: {status_to or status_from}. The destination may be physically unreachable.")
        ...
        # --- (Mission Log Generation is unchanged) ---
        ...
        mission_log.append({"Segment": "Takeoff Accel", "Time (s)": "N/A", "Energy (Wh)": f"{accel_e:.3f}", "Payload (kg)": current_payload, "Notes": "Inertial cost"})
        for i in range(1, len(full_path)):
            ...
            mission_log.append({"Segment": f"Leg {i}", "Time (s)": f"{t:.2f}", "Energy (Wh)": f"{e:.3f}", "Altitude (m)": f"{p2[2]:.1f}", "Distance (m)": f"{calculate_distance_3d(p1, p2):.1f}"})
        decel_e = predictor.calculate_inertial_energy(0); total_energy += decel_e
        mission_log.append({"Segment": "Landing Decel", "Time (s)": "N/A", "Energy (Wh)": f"{decel_e:.3f}", "Payload (kg)": 0, "Notes": "Inertial cost"})

        st.session_state.mission_plan = {
            'full_path': full_path, 'total_time': total_time, 'total_energy': total_energy, 'env': env,
            'mission_log': mission_log, 'solver_status': status_to if "Optimal" in status_to else status_from
        }
...
def results_stage():
    ...
    else: st.error(f"⚠️ Solver Status: {status}. The QUBO solver failed to produce a valid path.")
    ...
    with col1:
        ...
        if st.button("⬅️ New Mission", use_container_width=True): st.session_state.stage = 'setup'; st.rerun()
        with st.expander("Show Detailed Mission Log"): st.dataframe(pd.DataFrame(mission_plan['mission_log']).fillna(''), use_container_width=True)

    # --- (3D Visualization draw_scene function is unchanged) ---
    ...
    def draw_scene(animation_step):
        ...
        if path_len > 1 and max_time > 0:
            ...
            p_start, p_end = np.array(drone_path[current_segment_idx]), np.array(drone_path[current_segment_idx + 1])
        ...
        fig.add_trace(go.Scatter3d(x=full_3d_path[:,0], y=full_3d_path[:,1], z=full_3d_path[:,2], mode='lines', line=dict(width=4, color='yellow'), name='Optimal Path'))
