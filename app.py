...
import config; from environment import Environment, WeatherSystem; from ml_predictor.predictor import EnergyTimePredictor; from path_planner import PathPlanner3D
...
planner=load_planner();st.set_page_config(layout="wide");st.title("🚁 Q-DOP: Hybrid Hierarchical Planner (JPS+D* Lite)")
...
def planning_stage():
    mode=st.session_state.optimization_preference;log_event(f"Planning initial mission ('{mode}') with JPS...")
    with st.spinner(f"Hierarchical Planner running..."):
        m={"Fastest Path":"time","Most Battery Efficient":"energy","Balanced":"balanced"};sm=m[mode];bw=st.session_state.get('balance_weight',0.5)
        hub,order=st.session_state.hub_location,st.session_state.destination;payload=st.session_state.initial_payload=st.session_state.payload_kg
...
if planner.predictor.models:
    ...
    if st.session_state.mission_running:
        ...
        if idx<len(path)-1:
            drone_pos=st.session_state.drone_pos;planner.env.update_environment(st.session_state.total_time,0.5)
            if check_replanning_triggers():
                with st.spinner("D* Lite Replanning Current Segment..."):
                    new_path,status=planner.replan_path(
